...
def load_llm(model_name: str, hf_token: Optional[str], use_4bit: bool = True):
    ...
    if use_4bit and torch.cuda.is_available() and BitsAndBytesConfig is not None:
        log("Using 4-bit quantization to fit the model on A100 GPUs.")
...
Use ONLY the file and directory summaries below. Do not assume access to raw source code.

Generate a report-ready architectural description with these headings:

1. Architectural title
2. High-level cluster summary
3. Main responsibilities
4. Key components/classes
5. Interactions between components
6. Dependencies and external relationships
7. Why these files likely belong together architecturally
        ...
        cluster_rows.append(
            {
                ...
                "rsf_entities": len(entities),
                "matched_java_files": len(matched),
                "directory_summaries": len(dir_summaries),
                "summary_file": str(cluster_out.relative_to(work_dir)),
                "summary_preview": final_summary[:600].replace("\n", " "),
            }
        )
